...
async def create_guild_endpoint(data: GuildCreate, db: AsyncSession = Depends(get_session)):
...
async def list_guilds_endpoint(db: AsyncSession = Depends(get_session)):
    return await crud.list_guilds(db)


@router.get("/{guild_id}", response_model=GuildOut)
async def get_guild_endpoint(guild_id: int, db: AsyncSession = Depends(get_session)):
...
async def update_guild_endpoint(guild_id: int, data: GuildUpdate, db: AsyncSession = Depends(get_session)):
...
async def delete_guild_endpoint(guild_id: int, db: AsyncSession = Depends(get_session)):
