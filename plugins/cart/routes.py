...
async def create_new_cart(cart_data: CartCreate, db: AsyncSession = Depends(get_session)):
    cart = await create_cart(db, cart_data)
    return cart

# Get active cart by user
@router.get("/{user_id}", response_model=CartOut)
async def get_user_cart(user_id: int, db: AsyncSession = Depends(get_session)):
...
async def add_cart_item(cart_id: int, item: CartItemCreate, db: AsyncSession = Depends(get_session)):
    return await add_item_to_cart(db, cart_id, item)

# Remove item from cart
@router.delete("/items/{item_id}", response_model=dict)
async def remove_cart_item(item_id: int, db: AsyncSession = Depends(get_session)):
